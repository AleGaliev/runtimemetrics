...
func CreateMyHandler(storage Storage, connector connector, logger middleware.Logger, hashKey string, eventAudit *observer.Event, cryptoKey *crypto.Crypto) http.Handler {
	h := &MyHandler{
		storage:   storage,
		connector: connector,
		hashKey:   hashKey,
